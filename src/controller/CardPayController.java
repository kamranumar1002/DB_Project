...
*/
    ...
    void onbtPayAction(ActionEvent event) throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
    	...
    	String getTranasctionIDQuery = "SELECT top 1 id FROM transaction ORDER BY transaction.id DESC ";
    	new DBInitialize();
    	...
    	for(int i=0; i< purchasedproductitemsid.length; i++) {
    		...
    		String updatecountQuery = "EXEC UpdateProductCount '" + purchasedproductitemsid[i] + "', " + newcount;	
    		new DBInitialize().DBInitialize();
    		new DBInitialize();
