...
public class MainCashierController {
	...
	void initialize() throws ClassNotFoundException, SQLException, InstantiationException, IllegalAccessException {
		...
		String previousgetpurchaseid = " EXEC GetLatestPurchaseID";
		new DBInitialize();
		...
		String tablequery = "EXEC GetAllProducts";
	...
	void onbtPrintClick(ActionEvent event)
		...
		if (tf_change.getText().equals("") || tf_pay_amount.getText().equals("")) {
			...
			try {
				...
				String currenttime = currentTime.format(DateTimeFormatter.ofPattern("HH:mm:ss"));
				...
				String insertPurchaseQuery = "INSERT INTO purchase(id, date, time, cashierid, totalamount) "
						+ "VALUES (" + Common.purchaseid + ", '" + purchasedate + "', '" + currenttime + "', '"
						+ Common.cashierrec.getId() + "', " + Common.totalAmount + ")";
				DBInitialize.statement.executeUpdate(insertPurchaseQuery);

				// Insert each item into transaction_items table
				for (Sale saleItem : saledata) {
					String insertItemQuery = "INSERT INTO purchaseitems(purchase_id, barcode, quantity) VALUES ("
							+ Common.purchaseid + ", '" + saleItem.getBarcode() + "', " + saleItem.getQuantity() + ")";
					DBInitialize.statement.executeUpdate(insertItemQuery);

					// Update product count in productitems table
					String getOldCountQuery = "SELECT count FROM productitems WHERE barcode = '" + saleItem.getBarcode() + "'";
					ResultSet rsCount = DBInitialize.statement.executeQuery(getOldCountQuery);
					int oldCount = 0;
					if (rsCount.next()) {
						oldCount = rsCount.getInt("count");
					}
					int newCount = oldCount + saleItem.getQuantity();
					String updateCountQuery = "EXEC UpdateProductCount '" + saleItem.getBarcode() + "', " + newCount;						
				...
				// set slip number
				new DBInitialize().DBInitialize();
				String getpreviouspurchaseid = " EXEC GetLatestPurchaseID ";
				new DBInitialize();
